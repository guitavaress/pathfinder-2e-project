...
- ITEMS: the player can only use items listed in their Equipment (or looted on-screen). CHECK the Equipment line FIRST whenever the player declares an item. If they don't have it, resolve the REST of their declared actions normally and simply don't resolve the item — the engine tells the narrator that unresolved declarations didn't happen. Item use is never free:
